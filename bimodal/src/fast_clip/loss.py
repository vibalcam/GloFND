...
from fast_clip.fnd import LambdaThreshold
from fast_clip import log_utils
...
def gather_features(
    ...
    if use_horovod:
        ...
        if gather_with_grad:
            all_image_features = torch.cat(torch.distributed.nn.all_gather(image_features, feature_process_group), dim=0)
            all_text_features = torch.cat(torch.distributed.nn.all_gather(text_features, feature_process_group), dim=0)
        else:
            gathered_image_features = [torch.zeros_like(image_features) for _ in range(world_size)]
            gathered_text_features = [torch.zeros_like(text_features) for _ in range(world_size)]
            dist.all_gather(gathered_image_features, image_features, feature_process_group)
            dist.all_gather(gathered_text_features, text_features, feature_process_group)
            if not local_loss:
                # ensure grads for local rank when all_* features don't have a gradient
                gathered_image_features[rank] = image_features
                gathered_text_features[rank] = text_features
...
def log_hist_as_image(args, data, title, bins=64, dpi=300):
...
class ClipLoss(nn.Module):
    ...
    def __init__(
        ...
        self.feature_process_group = feature_process_group
        if not has_distributed or not dist.is_initialized():
            self.rank = 0
            self.world_size = 1
        else:
            self.rank = dist.get_rank(feature_process_group)
            self.world_size = dist.get_world_size(feature_process_group)
            logging.info(f"Feature process group: rank: {self.rank}, world_size: {self.world_size}")
...
class FastCLIPLoss(nn.Module):
    ...
    def local(self,
        ...
        results = self.pairwise_loss(features, remote_features, logit_scale, offset,
    ...
    def forward(self,
        ...
        results = self.pairwise_loss(remote_features, features, logit_scale, offset, sim,
        ...
        if self.multiply_tau:
            ...
            loss = loss + torch.mean(torch.log(u_im + self.eps) + torch.log(u_tt + self.eps)) / 2 / logit_scale
...
class FastCLIPLossGloFND(FastCLIPLoss):
    def __init__(self,
                 data_size: int,
                 device: torch.device = torch.device("cuda"),
                 **kwargs
        ...
        self.lda_tt = LambdaThreshold(device=device, N=data_size, **kwargs)
        self.lda_im = LambdaThreshold(device=device, N=data_size, **kwargs)
        self.epoch = 0
    ...
    def set_params(self,
                   image_idx: Optional[Tensor] = None, text_idx: Optional[Tensor] = None,
                   u_im: Optional[Tensor] = None, u_tt: Optional[Tensor] = None,
                   v_grad_im: Optional[Tensor] = None, v_grad_tt: Optional[Tensor] = None,
                    m_grad_im: Optional[Tensor] = None, m_grad_tt: Optional[Tensor] = None,
                    lda_im: Optional[Tensor] = None, lda_tt: Optional[Tensor] = None,
                   **kwargs
                   ):
        src_im_list = [u_im, v_grad_im, m_grad_im, lda_im]
        dst_im_list = [self.u_im, self.lda_im.v_grad, self.lda_im.m_grad, self.lda_im.lda]
        src_tt_list = [u_tt, v_grad_tt, m_grad_tt, lda_tt]
        dst_tt_list = [self.u_tt, self.lda_tt.v_grad, self.lda_tt.m_grad, self.lda_tt.lda]
        for src_im, dst_im, src_tt, dst_tt in zip(src_im_list, dst_im_list, src_tt_list, dst_tt_list):
            if src_im is not None:
                # assert image_idx is not None and dst_im.device == image_idx.device
                dst_im[image_idx] = src_im.to(dst_im.device)
            if src_tt is not None:
                # assert text_idx is not None and dst_tt.device == text_idx.device
                dst_tt[text_idx] = src_tt.to(dst_tt.device)
    ...
    def pairwise_loss(self,
                      ...
                      ref_features2: Optional[Tuple[Tensor, Tensor]] = None,
                      ref_sim: Optional[Tuple[Tensor, Tensor]] = None,
                      mask_lda: Optional[Tuple[Tensor, Tensor]] = None,
                      sum_lda: Optional[Tuple[Tensor, Tensor]] = None,
                      update_lda: bool = True,
                      ):
        image_features1, text_features1 = features1[0], features1[1]
        image_features2, text_features2 = features2[0], features2[1]
        image_idx, text_idx = indices[0], indices[1]
        ...
        # update lambda
        if update_lda:
            log_update_im = self.lda_im.update(sim=[sim_image], idx=image_idx, neg_self_mask=mask, epoch=self.epoch)
            log_update_tt = self.lda_tt.update(sim=[sim_text], idx=text_idx, neg_self_mask=mask, epoch=self.epoch)
            log_utils.update_dict(log_dict, log_update_im, sufix="_im")
            log_utils.update_dict(log_dict, log_update_tt, sufix="_tt")
            log_utils.update_dict(log_dict, {
                "lda_lr": self.lda_im.lr_lda,
                "lda": self.lda_im.lda[image_idx],
                "lda_v_grad": self.lda_im.v_grad[image_idx],
            }, sufix="_im")
            log_utils.update_dict(log_dict, {
                "lda_lr": self.lda_tt.lr_lda,
                "lda": self.lda_tt.lda[text_idx],
                "lda_v_grad": self.lda_tt.v_grad[text_idx],
            }, sufix="_tt")
        
        if mask_lda is not None:
            mask_im, mask_tt = mask_lda[0], mask_lda[1]
            sum_im, sum_tt = sum_lda[0], sum_lda[1]
            if mask_im.shape[1] > batch_size2:
                mask_im = mask_im[:, offset:offset + batch_size2]
                mask_tt = mask_tt[:, offset:offset + batch_size2]
        else:
            # compute masks
            mask_im, sum_im, log_mask_im = self.lda_im.get_mask(sim=sim_image, idx=image_idx, neg_self_mask=mask, epoch=self.epoch, labels=None)
            mask_tt, sum_tt, log_mask_tt = self.lda_tt.get_mask(sim=sim_text, idx=text_idx, neg_self_mask=mask, epoch=self.epoch, labels=None)
            log_utils.update_dict(log_dict, log_mask_im, sufix="_im")
            log_utils.update_dict(log_dict, log_mask_tt, sufix="_tt")
        ...
        # apply lda mask and ensure mask is also correct
        assert torch.all(mask_tt[mask == 0] == 0) and torch.all(mask_im[mask == 0] == 0), "lda mask should be 0 where mask is 0"
        exp_diff_image = torch.mul(exp_diff_image, mask_im)
        exp_diff_text = torch.mul(exp_diff_text, mask_tt)
        # exp_diff_image = torch.mul(exp_diff_image, mask)
        # exp_diff_text = torch.mul(exp_diff_text, mask)

        if batch_size1 <= batch_size2:
            # local vs global
            real_weights_sum_tt = sum_tt
            real_weights_sum_im = sum_im
        else:
            # global vs local
            real_weights_sum_tt = batch_size2 * sum_tt / batch_size1
            real_weights_sum_im = batch_size2 * sum_im / batch_size1
        loss_image = torch.sum(exp_diff_image, dim=-1, keepdim=True) / real_weights_sum_im
        loss_text = torch.sum(exp_diff_text, dim=-1, keepdim=True) / real_weights_sum_tt

        results = {"loss_image": loss_image, "loss_text": loss_text, "sim_image": sim_image, "sim_text": sim_text,
                   "exp_diff_image": exp_diff_image, "exp_diff_text": exp_diff_text, "diff_image": diff_image,
                   "diff_text": diff_text, "bounds_image": bounds_image, "bounds_text": bounds_text,
                   "mask": mask,
                   "mask_im": mask_im, "mask_tt": mask_tt, 
                   "sum_im": sum_im, "sum_tt": sum_tt, "log_dict": log_dict}
        return results
    ...
    def local(self,
        ...
        results = self.pairwise_loss(features, remote_features, indices, logit_scale, offset,
            update_lda=True, ref_features1=ref_features, ref_features2=ref_remote_features)
        loss1_im, loss1_tt = results["loss_image"], results["loss_text"]
        sim_im, sim_tt = results["sim_image"], results["sim_text"]
        mask_im, mask_tt = results["mask_im"], results["mask_tt"]
        sum_im, sum_tt = results["sum_im"], results["sum_tt"]
        ...
        gather_list = [
            u_im, u_tt, 
            self.lda_im.v_grad[image_idx], self.lda_tt.v_grad[text_idx],
            self.lda_im.m_grad[image_idx], self.lda_tt.m_grad[text_idx],
            self.lda_im.lda[image_idx], self.lda_tt.lda[text_idx],
        ]

        return loss1_im, loss1_tt, sim_im, sim_tt, [mask_im, mask_tt], [sum_im, sum_tt], gather_list, results["log_dict"]

    def forward(self,
                features: Tuple[Tensor, Tensor],
                remote_features: Tuple[Tensor, Tensor],
                remote_indices: Tuple[Tensor, Tensor],
                remote_u: Tuple[Tensor, Tensor],
                loss1: Tuple[Tensor, Tensor],
                u: Tuple[Tensor, Tensor],
                sim: Tuple[Tensor, Tensor],
                lda_mask: Tuple[Tensor, Tensor],
                lda_sum: Tuple[Tensor, Tensor],
                logit_scale: Tensor,
                offset: int,
                output_dict: bool = False,
                ref_features: Optional[Tuple[Tensor, Tensor]] = None,
        ...
        results = self.pairwise_loss(remote_features, features, remote_indices, logit_scale, offset, sim,
                                     mask_lda=lda_mask, sum_lda=lda_sum, update_lda=False,
                                     ref_features1=ref_remote_features, ref_features2=ref_features)
        loss2_im, loss2_tt = results["loss_image"], results["loss_text"]
