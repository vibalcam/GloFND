...
from .loss import ClipLoss, DistillClipLoss, CoCaLoss, SigLipLoss, FastCLIPLoss, FastCLIPLossIndividual, FastCLIPLossGloFND
...
def create_loss(args, feature_process_group = None):
    if args.distill:
        ...
        if "global" in args.temperature_scheme:
            if args.glofnd != 'none':
                loss_args.update({
                    "glofnd_type": args.glofnd,
                    "alpha": args.glofnd_alpha,
                    "start_update": args.glofnd_start_update,
                    "lda_start": args.glofnd_start_lda,
                    "lr_lda": args.glofnd_lr_lda,
                    "clip_grad_mult": args.glofnd_clip_grad_mult,
                })
                logging.info("Using FastCLIPLossGloFND")
                return FastCLIPLossGloFND(**loss_args)
            else:
                logging.info("Using FastCLIPLoss")
                return FastCLIPLoss(**loss_args)
        elif "individual" in args.temperature_scheme:
            assert args.glofnd == 'none'
    ...
    return ClipLoss(
        ...
        cache_labels=True,
        use_horovod=args.horovod,
        feature_process_group=feature_process_group,
    )
