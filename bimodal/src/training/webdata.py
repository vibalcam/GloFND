...
def read_cached_features(path: str):
    features = torch.load(path, map_location="cpu")
    return features


@torch.no_grad()
def get_cached_features(index: int|torch.Tensor, cached_features_dir: str, num_samples_per_shard: int = 10000):
    tar_index = index // num_samples_per_shard
    offset = index % num_samples_per_shard
    features = None
    if isinstance(index, int):
        tar_path = f"{cached_features_dir}/{tar_index:08d}.pt"
        features = read_cached_features(tar_path)[offset]       # shape: (2, dim)
    else:
        for i in tar_index.unique().tolist():
            tar_path = f"{cached_features_dir}/{tar_index:08d}.pt"
            cached_features = read_cached_features(tar_path)
            if features is None:
                features = torch.zeros(index.shape[0], 2, cached_features.shape[-1])
            features[tar_index == i] = cached_features[offset[tar_index == i]]       # shape: (batch, 2, dim)
        if features is not None:
            features = features.movedim(1, 0)
    return features
        ...
        if current_sample is None or prefix != current_sample["__key__"] or suffix in current_sample:
            if valid_sample(current_sample):
                if cached_features_dir:
                    ...
                    features = get_cached_features(index, cached_features_dir, num_samples_per_shard)
