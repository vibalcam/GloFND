...
def get_wds_dataset(args, preprocess_fns, is_train, epoch=0, tokenizer=None):
    ...
    if args.cache_ref_model_features:
        ...
        # split by node
        shards_list.sort(key=extract_shard_index)
        shards_list_rank = []
        num_shards_per_rank = (len(shards_list) + args.world_size - 1) // args.world_size
        for i in range(args.world_size):
            shards_list_rank.append(shards_list[i * num_shards_per_rank: min(len(shards_list), (i+ 1) * num_shards_per_rank)])
        shards_list = shards_list_rank[args.rank]
        args.data_size = len(shards_list) * args.num_samples_per_shard
        args.train_num_samples = args.data_size
        args.ref_features_offset = args.ref_features_offset + args.rank * num_shards_per_rank * args.num_samples_per_shard
        logging.info("Using validation image augmentation for reference model feature caching.")
    ...
    dataset = WebDataset(shards_list, is_train and not args.cache_ref_model_features, args.batch_size,
                         preprocess_img, args.seed, epoch,
                         functools.partial(tokenize, tokenizer) if tokenizer is not None else None,
                         return_index, not args.cache_ref_model_features, args.cached_ref_features_dir,
