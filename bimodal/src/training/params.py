...
def parse_args(args):
    ...
    parser.add_argument("--comm_freq", type=int, default=1,
                        help=("Frequency of communication of model and/or optimizer. If < 0, then communication"
                              " will be done at each iteration."))
    parser.add_argument("--num_sync_workers", type=int, default=-1,
                        help=("Number of workers in group to communicate gradients at each iteration."
                              " The world will then be divided into world_size // num_sync_workers groups, and"
                              " each worker will communicate gradients with the workers in its group at each iteratoin."
                              " If <= 0, then will be set to world size."))
    parser.add_argument("--num_feat_sync_workers", type=int, default=-1,
                        help=("Number of workers in group to communicate features at each iteration."
                              " Similar to num_sync_workers. If <= 0, then will be set to num_sync_workers."))
    parser.add_argument("--reducer", type=str, default="model",
                        help=("Which data is to be reduced, can be combination of ['model', 'optimizer', 'runtime']."
                              "Only works when num_sync_workers > 1. 'runtime' means the reducer is constructed when"
                              " needed, and is released immediately after the reduction."))
    parser.add_argument("--reduction_strategy", type=str, default="naive",
                        help=("Reduction strategy for the reducer, can be ['naive', 'inner_outer']."
                              " 'naive' means the model and/or optimizer is reduced every comm_freq iterations."
                              " 'inner_outer' means the difference between local model and previous global model"
                              " is viewed as a gradient, and will be updated using an outer optimizer."))
    parser.add_argument("--outer_optimizer", type=str, default="nesterov",
                        help="Outer optimizer when --reduction_strategy is set to 'inner_outer'.")
    parser.add_argument("--outer_lr", type=float, default=0.7, help="Learning rate for the outer optimizer.")
    parser.add_argument("--outer_momentum", type=float, default=0.9, help="Momentum for the outer optimizer.")
    ...
    parser.add_argument("--stop_iters", type=int, default=-1,
                        help=("Stop training after this many iterations."
                              " If given, the learning rate schedule will be based on this quantity."))
    ...
    # GloFND Arguments
    parser.add_argument("--glofnd", default='none', type=str, choices=["none", "fnc", "glofnd", "oracle"])
    parser.add_argument("--glofnd_reset_lda", default=False, action="store_true", help="Whether to reinitialize lambda when resuming")
    parser.add_argument("--glofnd_alpha", default=0.0, type=float)
    parser.add_argument("--glofnd_start_update", default=0, type=int)
    parser.add_argument("--glofnd_start_lda", default=0, type=int)
    parser.add_argument("--glofnd_lr_lda", default=0.05, type=float)
    parser.add_argument("--glofnd_clip_grad_mult", default=0.0, type=float)
